...
(function(){
  ...
  /* ========================================================
     03. ПОТЕРИ И ПРИОБРЕТЕНИЯ

     Четырнадцать строк двумя панелями — на телефоне это шесть
     экранов подряд. В каждой панели на виду остаются первые
     четыре: там поиск, запись, цена и реклама — то, ради чего
     панель собрана. Остальные три подтверждают уже сказанное
     и уходят под спойлер.

     Спойлер кладётся в свой <li>: перечень размечен списком,
     и <details> прямым ребёнком <ul> стоять не может.
     ======================================================== */
  (function(){
    [['.duel-c.neg', 'Ещё три строки счёта'],
     ['.duel-c.pos', 'Ещё три, что появятся вместе с сайтом']]
    .forEach(function(pair){
      var rows = items('s03', pair[0] + ' > ul > li');
      if (rows.length < 7) return;

      var d = spoiler(rows.slice(4), pair[1]);
      if (!d) return;

      var li = document.createElement('li');
      li.className = 'sp';
      d.parentNode.insertBefore(li, d);
      li.appendChild(d);
    });
  })();
  ...
  /* ========================================================
     ...
     Десять карточек столбиком — это пять экранов. На виду
     три пары: деньги за место, процент с записи и база
     пациентов. Отзывы и правила площадки уходят под спойлер:
     они подтверждают ту же мысль, а не открывают её.
     ======================================================== */
  (function(){
    var rows = items('s06', '.tally.duo > .tally-i');
    if (rows.length < 8) return;
    spoiler(rows.slice(6), 'Ещё отзывы и правила площадки',
            {count: rows.length - 6});
  })();
