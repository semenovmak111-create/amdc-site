...
(function(){
  ...
  /* ========================================================
     ...
     Двенадцать пунктов в трёх колонках. Каждая колонка
     становится спойлером, а её заголовок — надписью на нём.
     ======================================================== */
  (function(){
    items('s06', '.incl .col').forEach(function(col){
      var h = col.querySelector('h3'), ul = col.querySelector('ul');
      if (!h || !ul) return;
  ...
  /* ========================================================
     ...
     Карточки тарифов стоят друг под другом и занимают три
     экрана. На виду тот, который берут чаще, — он же средний;
     два других складываются, но цена каждого видна в
     заголовке спойлера.
     ======================================================== */
  (function(){
    var pl = items('s08', '.plans .plan');
    if (pl.length < 3) return;
    var pick = pl[1];
    pick.parentNode.insertBefore(pick, pl[0]);
    spoiler([pl[0]], 'Пять направлений — от 90 000 ₽', {count:false});
    spoiler([pl[2]], 'Четырнадцать и реклама — от 180 000 ₽', {count:false});
  })();

  /* 09. Под вопросами — единственное место, где предложение
     написать выглядит не продажей, а ответом на то, чего в
     списке не нашлось. */
  messengers(wrapOf('s09'),
    'Вопроса нет в списке? Напишите одной строкой.',
